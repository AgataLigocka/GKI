...
public class Lab5 extends GLJPanel implements GLEventListener, KeyListener{
    ...
    public void First(GL2 gl2, int tCount, int rad) {
    	gl2.glClear(GL2.GL_COLOR_BUFFER_BIT);
    	int tCountM = tCount*100;
    	double angleInc = Math.PI * tCount / tCountM;
    	float h = 0;
    	
    	gl2.glBegin(GL2.GL_POLYGON);
    	for (int i = 0; i < tCountM; ++i) {
    		angle = angleInc * Math.PI * i;
    		
    		gl2.glColor3f(0.5F * x1, (x1 * x2 / 2) / 0.3F, 0.4F * x2);
    		x1 = (float) (rad * Math.cos(angle));
    		x2 = (float) (rad * Math.sin(angle));
    		
    		gl2.glVertex3f(x1,  h,  x2);
    		h = h + tCount/1500F;
    ...
    public void Triangle(GL2 gl2){
        ...
        gl2.glVertex3f( -1f, -1.5f, 0.0f );
        
        gl2.glColor3f( 0.0f, 1.0f, 0.0f ); 
        gl2.glVertex3f( 0.0f, 4.0f, -3f );
        
        gl2.glColor3f( 0.5F, 0.0F, 0.5F );  // Purple
        gl2.glVertex3f( 1f, -1.5f, 0.0f );

        gl2.glEnd();
    }
    
    public void Second(GL2 gl2,double size){
        ...
        for ( int i = 0; i<n;i++)
        {
            ...
            gl2.glVertex3f((float) (Math.cos(i*2*Math.PI/n)*size), 
            			0, (float) (Math.sin(i*2*Math.PI/n)*size));
            
            gl2.glColor3f(1.0f, 0.0f,0.0f);
            gl2.glVertex3f((float) (Math.cos((i+1)*2*Math.PI/n)*size), 
            			0, (float) (Math.sin((i+1)*2*Math.PI/n)*size));
        }
        ...
        for (int i=0; i<n;i++)
        {
            ...
            gl2.glVertex3f((float) 
            			(Math.cos(i*2*Math.PI/n)*size), 
            			0, (float) 
            			(Math.sin(i*2*Math.PI/n)*size));
            gl2.glVertex3f((float) 
            			(Math.cos((i+1)*2*Math.PI/n)*size), 
            			0, 
	...
	public void display(GLAutoDrawable drawable) {    
		...
		gl2.glClearColor( 1, 1, 1, 1 ); // Background color (black).
